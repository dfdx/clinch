...
//		IndexReader ir = IndexReader.open(this.indexPath);
...
//	}
	...
	public Map<Integer, Integer> freqInDocs(String word) throws CorruptIndexException, IOException {
		Map<Integer, Integer> ret = new HashMap<Integer, Integer>();
		Term term = new Term(CONTENTS_FIELD, word);
		IndexReader ir = IndexReader.open(indexDir);
		try {
			TermDocs termDocs = ir.termDocs(term);
			while (termDocs.next()) {
				ret.put(termDocs.doc(), termDocs.freq());
			}
		} finally {
			ir.close();
		}
